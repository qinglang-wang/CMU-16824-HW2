...
def train_model(
    ...
    prefetcher = Prefetcher(train_loader, 'cuda')
    while iters <= num_iterations:
        train_batch = prefetcher.next()
        with torch.amp.autocast('cuda', enabled=amp_enabled):
            ...
            ##################################################################
            generated_batch = gen.forward(train_batch.shape[0]).detach()
            discrim_real = disc.forward(train_batch)
            discrim_fake = disc.forward(generated_batch)
            ##################################################################
            #                          END OF YOUR CODE                      #
            ##################################################################
            ...
            # discriminator on it.
            ###################################################################
            epsilon = torch.rand(train_batch.shape[0], 1, 1, 1, device='cuda')
            
            interp = epsilon * train_batch + (1 - epsilon) * generated_batch
            interp.requires_grad_(True)

            with torch.amp.autocast('cuda', enabled=amp_enabled):
                discrim_interp = disc.forward(interp)
            # discrim_interp, interp = None, None
            ##################################################################
            #                          END OF YOUR CODE                      #
            ##################################################################
        ...
        if iters % log_period == 0 and iters != 0:
            with torch.no_grad():
                with torch.amp.autocast('cuda', enabled=amp_enabled):
                    ...
                    ##################################################################
                    generated_samples = (gen.forward(100) + 1) / 2
                    ##################################################################
                    #                          END OF YOUR CODE                      #
                    ##################################################################
                save_image(
